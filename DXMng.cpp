...
void ___Trace(const char *format, ...)
{
	va_list argptr;
	char    buf[256];
...
BOOL ___DDError(HRESULT hErr, char* file, int line)
{
	...
	TRACE1("%s\n", err);	
	int rval = MessageBox(hWnd, err, "실행을 종료하겠습니까?", MB_YESNO|MB_TOPMOST);
    if (rval == IDYES) PostMessage(hWnd, WM_CLOSE, 0, 0);
